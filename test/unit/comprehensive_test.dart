// Комплексные unit тесты для Фаз 2-4
...
void main() {
  group('Phase 2: Auth Module', () {
    ...
    test('TestAuthClient автоматически refresh токен', () async {
      final auth = TestAuthClient();

      // Создаём токен с коротким TTL
      final shortToken = await auth.loginWithCredentials('test@example.com', 'password');
      auth.setToken(AQToken(
        rawJwt: 'short-lived',
        claims: shortToken.claims,
        expiresAt: DateTime.now().add(const Duration(seconds: 3)),
      ));

      // Ждём refresh (происходит за 2 минуты до истечения, но для теста используем короткий TTL)
      auth.events.listen((event) {
        // Просто слушаем события
      });

      // Для реального теста нужно подождать, но это займёт время
      // В production тесте можно использовать fake timer
      expect(auth.currentToken, completes);
    });
